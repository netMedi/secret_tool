  ...
  while [ "$xyz" = "" ]; do
    ...
    case "$xyz" in
      [Yy]* )
        ...
        op item edit --vault Private SECRET_TOOL version="$CURRENT_TOOL_VERSION" 2> /dev/null
        exit $?
        ;;
      [Zz]* )
        echo '[INFO] setting approved version to "latest"...'
        op item edit --vault Private SECRET_TOOL version=latest 2> /dev/null
        exit $?
        ;;
      * )
...
sudo sh -c "mkdir -p '$SECRET_TOOL_DIR_INSTALL'; cp '$SECRET_TOOL_DIR_SRC/$SECRET_TOOL_EXE' '$SECRET_TOOL_DIR_INSTALL/secret_tool' && chmod +x '$SECRET_TOOL_DIR_INSTALL/secret_tool'" \
  ...
  || {
    ...
    echo
    echo "  alias secret_tool=\"$SECRET_TOOL_DIR_SRC/dist/secret_tool\""
