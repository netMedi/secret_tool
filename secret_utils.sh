...
case $routine in
  ...
  install)
    ...
    sudo sh -c "ln -s $script_dir/secret_tool.sh ${SYMLINK_DIR:-/usr/local/bin}/secret_tool && chmod +x ${SYMLINK_DIR:-/usr/local/bin}/secret_tool" && echo '[DONE] Secret tool has been installed. You may need to restart terminal if the "secret_tool" command is not immediately available' || echo '[ERROR] Failed to install secret tool'
    ;;
  uninstall)
